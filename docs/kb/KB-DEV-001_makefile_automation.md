...
| `make lang` | `configure_target_languages()` 실행 후 `make po` | 컨테이너 재생성으로 지역 기본값·MO가 유실됐을 때 |
...
3. **`make lang` 은 지역 기본값을 다시 씁니다.** `configure_target_languages()` 는 `force_defaults=False` 로 호출되므로 `System Settings.language` 가 이미 설정돼 있으면 덮어쓰지 않습니다 (KB-LOC-001 §2).
