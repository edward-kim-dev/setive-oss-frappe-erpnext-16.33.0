...
verified_by: 개발 컨테이너 E2E — 제조·상품매매 fixture 회사(scripts/nts/fixture_company.py) 각 17·13 전표(테넌트 추가계정·업종 프로필 전환 포함)로 리포트 3종 실행, 대사식 9/9 통과(차이 0원)
...
매핑표를 고치지 않고 엔진 상수로 얹는 보정은 `nts_report.py` 모듈 docstring의 **D-1 ~ D-7** 입니다. 적용 사실은 매 실행 `spec_patch` 등급 info 경고에 남습니다.
...
폴백이 실제로 문제가 되는 범위는 좁습니다. ERPNext 는 영구재고 회사에서 재고(Stock) 계정에 Journal Entry 를 직접 기표하지 못하게 막으므로(`journal_entry.py` `validate_stock_accounts` → `StockAccountInvalidTransaction`) 재고 흐름 측정에는 폴백이 오지 않고, 제조원가 계정의 흡수 전 총액 측정(D-2)에서만 옵니다 — 그 경우 상대가 전부 결제계정이라 값은 맞습니다(제조 fixture: 임금 JE `1111, 1115` 나열 → 경고는 나오되 노무비 9,000 정확).
...
엔진은 그 대체를 **Ⅷ.타계정대체액(mfg 48)** 에 합산합니다 — `MOVEMENT_PATCH[("mfg","48")]["sets_add"] = ["RM"]`. 당기재료매입액(3)에서 빼지 않는 이유: 3 은 매입 세금계산서 합계와 대조되는 행이라 실매입액을 유지해야 하고, 48 은 "제조원가 중 제품이 되지 않은 것" 이라는 뜻에 맞습니다. 결과적으로 재료비(1)와 당기총제조비용(44)은 투입 전 총액을, 49 는 정확한 당기제품제조원가를 보입니다. 제조 fixture: 원재료 5,000 견본 → mfg 1 = 45,000 · 44 = 54,000 · 48 = 5,000 · 49 = 49,000. 세무 판단이 갈릴 수 있어 매핑표가 아니라 엔진 상수에 두었습니다.
...
| 회사 | 흐름 |
|---|---|
| `세티브 (Fixture-MFG)` | 원재료 매입 100,000 · 환출 10,000 · 매입(update_stock) 20,000 → 임금 6,000 + 3,000(현금·예금 다계정 JE, 폴백 경로) → Stock Entry 이동 40,000 → 제조(제품 20개 = 재료 40,000 + 노무비 배부 5,000) → DN 15개 + SI · SI(update_stock) 2개 → 반품 1개 → 제품 견본 1개(5328) · 원재료 견본 5개(D-7) → 정상감모 1개(5140) → 평가손실 500(5141) → 비정상감모 1개(표준 5460 → is 188 영업외비용) |
...
|---|---|---|
| 원장 (`assigned` + 5110/5120 + 5140 + 5141 + 5150 + 미흡수 MFG_COST) | 0 + 36,000 + 2,250 + 500 + 0 + 4,000 = **42,750** | 2,000(5160) + 68,000 + 1,000 + 700 + 300 = **72,000** |
| 서식 산식 총계 (is.35) | **42,750** | **72,000** (36 70,000 + 48 공사원가 2,000) |
| 내역 | 43 0 + 44 49,000 − 46 1,750 − 47 4,500(제품 견본 2,250 + 비정상감모 2,250) | 37 0 + 38 110,300(5150 잔액 300 포함) − 40 37,300 − 41 3,000 |
| 제조원가명세서 | 재료비 45,000(매입 110,000 − 기말 65,000) · 노무비 9,000 · 44 54,000 · 48 5,000(원재료 견본) · 49 **49,000** | — |
| 경고 | `counter_fallback` 1건(다계정 JE — 값은 정확) | 없음 |
...
4. **감모손실의 정상·비정상** — 표준 계정과목표가 이미 구분합니다: `5140 재고자산감모손실`(정상 → 매출원가 흡수) · `5460 재고자산감모손실(비정상)`(→ is 188 영업외비용). Stock Reconciliation 의 차이계정으로 5460 을 고르면 엔진은 재고 대변의 상대가 5460 이라 타계정대체(is 47)로 잡고, 5460 은 영업외비용에 실립니다(제조 fixture, 9/9). 오전 판 §8-4 는 "5140 하나" 라고 틀리게 적었습니다(§9). 테넌트 추가계정을 Custom Field 로 직접 배정하는 경로는 상품매매 fixture 의 `5160 공사원가`(→ is 50, COA 에 없는 번호) 가 검증합니다.
5. **D-7 의 세무 확인** — 원재료 타계정대체를 mfg 48 에 합산하는 처리(§4.4)는 세무 판단이 갈릴 수 있습니다. 당기재료매입액(3) 차감을 요구받으면 `MOVEMENT_PATCH` 의 `sets_add` 를 옮기면 됩니다.
...
7. **GL 병합으로 `against` 가 유실될 수 있습니다** — 한 전표에서 같은 재고계정·같은 원가중심의 라인 둘이 서로 다른 상대계정을 가지면 병합되어 하나만 남습니다([KB-ARCH-002](./KB-ARCH-002_erpnext_frappe_customization_mechanics.md) §3.4). 둘 다 매출원가 계정이면 블록 총계는 영향이 없고, 하나가 타계정대체 대상(견본비 등)이면 분류가 틀리지만 그때는 `매출원가 항등식` 이 반드시 실패해 검출됩니다 — 조용히 틀리지는 않습니다. 다만 병합은 **원가중심·프로젝트를 쓰지 않는 테넌트에서만** 일어나므로 한 테넌트 검증으로 일반화하면 안 됩니다.
...
- **§8-7 (2026-09-08 오전 판) "Journal Entry 로 재고계정을 직접 기표하면 비례 배분으로 폴백한다"** — 과장이었습니다. ERPNext 가 영구재고 회사의 재고계정 JE 를 막으므로(§4.3) 재고 흐름에는 폴백이 오지 않습니다. 폴백은 제조원가 총액 측정(D-2)에서만 오고 값은 맞습니다.
