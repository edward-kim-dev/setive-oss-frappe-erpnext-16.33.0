...
| **3** | **사업자번호 단위** 값 (CorpNum, 연동회원 상태, 인증서 상태, 업태·종목) | **Company Link 일반 DocType** | 사업자번호가 Company 단위 |
...
## 6. 3층 — Company 당 1행 (합치지 않는다)

`Korea Integration Account` (일반 DocType, `autoname: format:{provider}-{company}`)

| 필드 | 타입 | 비고 |
|---|---|---|
| `provider` · `company` | Select · Link(Company) | reqd. **company 와 corp_num 양쪽에 unique 제약** |
| `corp_num` | **Data** | 사업자등록번호. **비밀이 아니다.** validate 에서 하이픈 제거·체크섬 검증 + `Company.tax_id` 불일치 시 throw |
| `user_id` | Data | 연동회원 담당자 아이디. 비밀 아님 |
| `api_secret` · `cert_password` | **Password** | `__Auth` 로 감. `get_password()` 로만 읽는다 |
| `biz_type` · `biz_class` · `ceo_name` | Data | `joinMember` 필수인데 Company 에 없는 필드 |
...
| `get_account(company, provider=None)` | 없으면 한국어로 throw |
